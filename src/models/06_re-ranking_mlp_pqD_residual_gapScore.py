...
LOG_PATH = os.path.join(OUTPUT_DIR, "logs", "06_re-ranking_mlp_pqD_residual_gapScore.csv")
...
if not os.path.exists(INPUT_PATH):
    ...
    print(f"  (참고: 직전 코드생성 파일명은 '03_re-ranking_features_pqD_residual_gapScore.npz' 였습니다.)")
    exit()

data = np.load(INPUT_PATH)
dataset = data['data'] 

# [수정] Feature 개수가 33개인지 확인 (0~15:PQ, 16~31:Resid, 32:Gap, 33:Label)
...
# [핵심 수정] 3개 그룹 분리 및 개별 스케일링 적용
print("  >>> Feature 그룹별(PQ Dist, Residual, Gap) 개별 Scaling 적용 중...")

# 1) PQ Distance Features (Col 0 ~ 15)
X_train_pq = X_train_raw[:, :16]
X_val_pq   = X_val_raw[:, :16]

# 2) Residual Features (Col 16 ~ 31)
X_train_res = X_train_raw[:, 16:32]
X_val_res   = X_val_raw[:, 16:32]

# 3) Gap Score Feature (Col 32)
X_train_gap = X_train_raw[:, 32:] # (N, 1) 유지
...
# 다시 병합 (Concatenate: 16 + 16 + 1 = 33)
X_train = np.hstack([X_train_pq_scaled, X_train_res_scaled, X_train_gap_scaled])
X_val   = np.hstack([X_val_pq_scaled, X_val_res_scaled, X_val_gap_scaled])
...
print(f"  - Gap Score Mean (Train): {np.mean(X_train_gap_scaled):.4f}")
...
class SimpleMLP(nn.Module):
    def __init__(self):
        ...
        self.network = nn.Sequential(
            nn.Linear(33, 16), # 입력이 많아져서 첫 은닉층을 16으로 설정
            nn.ReLU(),
            nn.Linear(16, 8),
            nn.ReLU(),
            nn.Linear(8, 1),
