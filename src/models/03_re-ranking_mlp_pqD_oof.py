...
# [수정] 입력 파일 유지, 출력 경로 이름 변경 (pqD_only 버전)
INPUT_PATH = os.path.join(INPUT_DIR, "03_re-ranking_features_pqD_residual.npz")
LOG_PATH = os.path.join(OUTPUT_DIR, "logs", "03_re-ranking_mlp_pqD_only_oof.csv")
OOF_PATH = os.path.join(OUTPUT_DIR, "oof", "03_re-ranking_mlp_pqD_only_oof.npz")
...
# 전체 33개 컬럼 (0~15: pqD, 16~31: residual, 32: Label)
# 변경: dataset[:, :16] -> 앞쪽 16개 (pqD only)
X_numpy = dataset[:, :16].astype(np.float32)
...
for fold in range(NUM_FOLDS):
    ...
    # Feature가 한 종류(pqD)만 있으므로 분할 없이 단일 Scaler 사용
    # -------------------------------------------------------------
    # 1) Raw 데이터 슬라이싱
