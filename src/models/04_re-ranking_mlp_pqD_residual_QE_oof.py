...
# [수정] 입력 파일 및 출력 경로 변경 (QE 버전, 48 Feature)
INPUT_PATH = os.path.join(INPUT_DIR, "04_re-ranking_features_pqD_residual_QE.npz")
LOG_PATH = os.path.join(OUTPUT_DIR, "logs", "04_re-ranking_mlp_pqD_residual_QE_oof.csv")
...
if not os.path.exists(INPUT_PATH):
    ...
    print("참고: 이전 단계에서 생성한 QE 버전 파일이 필요합니다.")
    exit()
...
for fold in range(NUM_FOLDS):
    ...
    # [핵심 수정] Fold별 독립적인 3-Group Split Scaling 적용
    # -------------------------------------------------------------
    # 1) Raw 데이터 슬라이싱
    ...
    # 2) Feature 그룹 분리 (각 16차원)
    # 0~15: PQ Dist / 16~31: Residual / 32~47: Quantization Error
    X_train_f1, X_train_f2, X_train_f3 = X_train_raw[:, :16], X_train_raw[:, 16:32], X_train_raw[:, 32:]
    X_val_f1, X_val_f2, X_val_f3 = X_val_raw[:, :16], X_val_raw[:, 16:32], X_val_raw[:, 32:]
    X_test_f1, X_test_f2, X_test_f3 = X_test_raw[:, :16], X_test_raw[:, 16:32], X_test_raw[:, 32:]
    ...
    # 5) 다시 병합 (Concatenate)
    X_train_scaled = np.hstack([X_train_f1_scaled, X_train_f2_scaled, X_train_f3_scaled])
    X_val_scaled = np.hstack([X_val_f1_scaled, X_val_f2_scaled, X_val_f3_scaled])
    X_test_scaled = np.hstack([X_test_f1_scaled, X_test_f2_scaled, X_test_f3_scaled])
    # -------------------------------------------------------------
