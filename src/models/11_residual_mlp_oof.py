...
# 모델 구조 출력 (한 번만)
temp_model = MultiTaskDistancePredictor()
print(temp_model)
...
for fold in range(NUM_FOLDS):
    ...
    y_local_train = (y_local[train_idx] - y_local_mean) / y_local_std
    y_local_val = (y_local[val_idx] - y_local_mean) / y_local_std
    ...
    with torch.no_grad():
        _, test_global_pred = model(X_test_tensor)
...
print(f"  Acc-like (10%):  {oof_metrics['acc_like']:.4f}")
print(f"{'='*70}")
