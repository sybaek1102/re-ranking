...
# [수정] 입력 파일 및 출력 경로 변경 (gtD_gapScore 버전)
INPUT_PATH = os.path.join(INPUT_DIR, "05_re-ranking_features_gtD_gapScore.npz")
LOG_PATH = os.path.join(OUTPUT_DIR, "logs", "05_re-ranking_mlp_gtD_gapScore_oof.csv")
OOF_PATH = os.path.join(OUTPUT_DIR, "oof", "05_re-ranking_mlp_gtD_gapScore_oof.npz")
...
if not os.path.exists(INPUT_PATH):
    ...
    print(f"  (참고: 직전 코드생성 파일명은 '05_re-ranking_features_gtD_gapScore.npz' 였습니다.)")
    exit()

data = np.load(INPUT_PATH)
dataset = data['data'] 

# [수정] Feature 개수가 17개인지 확인 (0~15: gtD, 16: GapScore, 17: Label)
X_numpy = dataset[:, :-1].astype(np.float32)
y_numpy = dataset[:, -1].astype(np.float32).reshape(-1, 1)

print(f"  - Total Feature Shape: {X_numpy.shape}") # (N, 17) 예상
...
class SimpleMLP(nn.Module):
    def __init__(self):
        ...
        # [수정] 입력 차원 16 -> 17 (Distance 16개 + Gap 1개)
...
for fold in range(NUM_FOLDS):
    ...
    # 2) Feature 그룹 분리 (Distance 16개 / Gap Score 1개)
    # Distance: 0~15
    # Gap Score: 16
    
    # Train
    X_train_dist = X_train_raw[:, :16]
    X_train_gap  = X_train_raw[:, 16:]
    ...
    # Test
    X_test_dist  = X_test_raw[:, :16]
    X_test_gap   = X_test_raw[:, 16:]
    ...
    # 5) 다시 병합 (Concatenate)
    X_train_scaled = np.hstack([X_train_dist_scaled, X_train_gap_scaled])
    X_val_scaled   = np.hstack([X_val_dist_scaled, X_val_gap_scaled])
    X_test_scaled  = np.hstack([X_test_dist_scaled, X_test_gap_scaled])
    # -------------------------------------------------------------
