...
# 직전 단계에서 생성한 Log Quantized 파일명으로 설정
FEATURE_PATH = os.path.join(INPUT_DIR, "13_residual_features_log_quantized.npz")
...
# 9:     centroid_mae (1)
# 10:    centroid_var (1)
# --- Base Ends (Indices 0-10) ---
# 11:    int8_scaled_sq (1)
# 12:    int8_scaled (1)
# 13:    int8_scaled_dot (1)
# --- INT8 Ends (Indices 11-13) ---
# 14:    int4_scaled_sq (1)
# 15:    int4_scaled (1)
# 16:    int4_scaled_dot (1)
# --- INT4 Ends (Indices 14-16) ---

FEATURE_CONFIGS = [
    {
        'name': 'int8_log_quantized',
        'indices': list(range(14)),  # 0~13 (Base + INT8 Log Features)
        'log_path': os.path.join(OUTPUT_DIR, "logs", "13_residual_mlp_log_quantized_int8.csv"),
        'description': 'Base + INT8 Log-Quantized Norm Features (14 dims)'
    },
    {
        'name': 'int4_log_quantized',
        'indices': list(range(11)) + [14, 15, 16],  # 0~10 (Base) + 14~16 (INT4 Log Features)
        'log_path': os.path.join(OUTPUT_DIR, "logs", "13_residual_mlp_log_quantized_int4.csv"),
        'description': 'Base + INT4 Log-Quantized Norm Features (14 dims)'
    }
]
...
# 데이터 로드
X_full_np = np.load(FEATURE_PATH)["data"].astype(np.float32)  # (160000, 16, 17)
...
class SingleTaskDistancePredictor(nn.Module):
    ...
    def forward(self, x):
        ...
        # 1. Flatten for Shared MLP: (batch * 16, feature_dim)
        x_flat = x.view(-1, self.feature_dim)
        x_norm = self.input_norm(x_flat)
        embeddings = self.shared_mlp(x_norm) # (batch * 16, embed_dim)
        
        # 2. Reshape & Flatten for Global MLP
        global_input = embeddings.view(batch_size, -1) # (batch, 16 * embed_dim)
...
for config in FEATURE_CONFIGS:
    ...
    # Train/Val Split for current features
    X_train = X_selected[train_idx]
    X_val = X_selected[val_idx]
    
    # DataLoader
    train_dataset = TensorDataset(torch.tensor(X_train), torch.tensor(y_global_train))
    val_dataset = TensorDataset(torch.tensor(X_val), torch.tensor(y_global_val))
