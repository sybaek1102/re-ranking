...
LOG_PATH = os.path.join(OUTPUT_DIR, "logs", "03_re-ranking_mlp_pqD_residual.csv")
...
print(f"  - Total Feature Shape: {X_numpy.shape}") # (N, 32)
...
# 그룹 분리 (앞 16개: PQ Dist / 뒤 16개: Residual)
...
class SimpleMLP(nn.Module):
    def __init__(self):
        ...
        self.network = nn.Sequential(
            nn.Linear(32, 8),  # 입력이 늘었으므로 은닉층도 약간 확장 (4->8) 추천, 원하면 4 유지 가능
            nn.ReLU(),
            nn.Linear(8, 1),
            nn.Sigmoid()
        )
