...
# [수정] 입력 파일 및 출력 경로 변경 (GapScore 추가 버전)
INPUT_PATH = os.path.join(INPUT_DIR, "06_re-ranking_features_pqD_residual_gapScore.npz")
LOG_PATH = os.path.join(OUTPUT_DIR, "logs", "06_re-ranking_mlp_pqD_residual_gapScore_oof.csv")
OOF_PATH = os.path.join(OUTPUT_DIR, "oof", "06_re-ranking_mlp_pqD_residual_gapScore_oof.npz")
...
if not os.path.exists(INPUT_PATH):
    ...
    # 직전 단계 파일명 확인용 메시지
    print(f"  (참고: 직전 코드생성 파일명은 '06_re-ranking_features_pqD_residual_gapScore.npz' 였습니다.)")
    exit()

data = np.load(INPUT_PATH)
dataset = data['data'] 

# [수정] Feature 개수가 33개인지 확인 (0~31: Feature 2종, 32: GapScore, 33: Label)
X_numpy = dataset[:, :-1].astype(np.float32)
y_numpy = dataset[:, -1].astype(np.float32).reshape(-1, 1)

print(f"  - Total Feature Shape: {X_numpy.shape}") # (N, 33) 예상
...
class SimpleMLP(nn.Module):
    def __init__(self):
        ...
        self.network = nn.Sequential(
            nn.Linear(33, 16), # 입력 확장 (32->33), 첫 은닉층 16
            nn.ReLU(),
            nn.Linear(16, 8),
            nn.ReLU(),
            nn.Linear(8, 1),
...
for fold in range(NUM_FOLDS):
    ...
    # [핵심 수정] Fold별 독립적인 Split Scaling 적용 (3개 그룹)
    # -------------------------------------------------------------
    # 1) Raw 데이터 슬라이싱
    ...
    # 2) Feature 그룹 분리 (PQ Dist / Residual / Gap Score)
    # PQ Dist: 0~15 (16개)
    # Residual: 16~31 (16개)
    # Gap Score: 32 (1개)
    
    # Train
    X_train_pq  = X_train_raw[:, :16]
    X_train_res = X_train_raw[:, 16:32]
    X_train_gap = X_train_raw[:, 32:]
    
    # Val
    X_val_pq  = X_val_raw[:, :16]
    X_val_res = X_val_raw[:, 16:32]
    X_val_gap = X_val_raw[:, 32:]
    
    # Test
    X_test_pq  = X_test_raw[:, :16]
    X_test_res = X_test_raw[:, 16:32]
    X_test_gap = X_test_raw[:, 32:]

    # 3) Scaler 정의 및 Train 데이터로 Fit (각각 별도로 수행)
    scaler_pq  = StandardScaler()
    scaler_res = StandardScaler()
    scaler_gap = StandardScaler()

    X_train_pq_scaled  = scaler_pq.fit_transform(X_train_pq)
    ...
    # Val
    X_val_pq_scaled  = scaler_pq.transform(X_val_pq)
    X_val_res_scaled = scaler_res.transform(X_val_res)
    X_val_gap_scaled = scaler_gap.transform(X_val_gap)
    
    # Test
    X_test_pq_scaled  = scaler_pq.transform(X_test_pq)
    X_test_res_scaled = scaler_res.transform(X_test_res)
    X_test_gap_scaled = scaler_gap.transform(X_test_gap)

    # 5) 다시 병합 (Concatenate)
    X_train_scaled = np.hstack([X_train_pq_scaled, X_train_res_scaled, X_train_gap_scaled])
    X_val_scaled   = np.hstack([X_val_pq_scaled, X_val_res_scaled, X_val_gap_scaled])
    X_test_scaled  = np.hstack([X_test_pq_scaled, X_test_res_scaled, X_test_gap_scaled])
    # -------------------------------------------------------------
